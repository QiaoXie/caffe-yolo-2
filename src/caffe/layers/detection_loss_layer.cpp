...
void DetectionLossLayer<Dtype>::Forward_cpu(
  ...
  for (int i = 0; i < bottom[0]->num(); ++i) {
    ...
    for (int j = 0; j < locations; ++j) {
      ...
      bool isobj = label_data[true_index + j];
      if (!isobj) {
        continue;
      }
      obj_count += 1;
      int label = static_cast<int>(label_data[true_index + locations  + j]);
//      CHECK_GE(label, 0) << "label start at 0";
  ...
  //    CHECK_LT(label, num_class_) << "label must below num_class";
      ...
      const Dtype* true_box_pt = label_data + true_index + locations * 2 + j * 4;
  ...
  float obj_count_f = obj_count + 0.01;
  class_loss /= obj_count_f;
  coord_loss /= obj_count_f;
  area_loss /= obj_count_f;
  obj_loss /= obj_count_f;
  ...
  LOG(INFO) << "avg_iou: " << avg_iou << " avg_obj: " << avg_obj << " avg_no_obj: "
        << avg_no_obj << " avg_cls: " << avg_cls << " avg_pos_cls: " << avg_pos_cls << " obj_count: "<<obj_count;
}
