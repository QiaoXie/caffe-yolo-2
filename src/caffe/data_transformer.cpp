...
void DataTransformer<Dtype>::Transform(const Datum& datum,
  ...
  CHECK_EQ(float_size % 5, 0) <<
    "Every box label has 5 labels (class,  box)";
  vector<BoxLabel> ori_labels;
  for (int j = 0; j < float_size; j += 5) {
    BoxLabel box_label;
  ...
  float rand_scale_w = (1. - Rand(30) / 100.);
  float rand_scale_h = (1. - Rand(30) / 100.);
  int rand_w = static_cast<int>(img_width * rand_scale_w) - 1;
  int rand_h = static_cast<int>(img_height * rand_scale_h) - 1;
...
//  LOG(INFO) << "img_width: " << img_width << " img_height: " << img_height;
  ...
  for (int i = 0; i < ori_labels.size(); ++i) {
    ...
    int ori_left = ori_x - ori_w / 2;
    int ori_top  = ori_y - ori_h / 2;
    int ori_right = ori_x + ori_w / 2;
    int ori_bottom = ori_y + ori_h / 2;
	

    int new_left = ori_left - rand_left;
    int new_top = ori_top - rand_top;
    int new_right = ori_right - rand_left;
	  int new_bottom = ori_bottom - rand_top;

    new_left = std::max(new_left, 0);
    new_top  = std::max(new_top, 0);
    new_right = std::min(new_right, rand_w);
	  new_bottom = std::min(new_bottom, rand_h);
    ...
    box_label.box_[0] = float(new_left + new_right ) / 2 / rand_w;
    box_label.box_[1] = float(new_top + new_bottom ) / 2 / rand_h;
    box_label.box_[2] = float( new_width ) / rand_w;
    box_label.box_[3] = float( new_height ) / rand_h;

    if(box_label.box_[0] < 0 || box_label.box_[1] < 0)
      continue;

    box_label.box_[0] = constrain(0.0f, 1.0f, box_label.box_[0]);
    box_label.box_[1] = constrain(0.0f, 1.0f, box_label.box_[1]);
    box_label.box_[2] = constrain(0.0f, 1.0f, box_label.box_[2]);
    box_label.box_[3] = constrain(0.0f, 1.0f, box_label.box_[3]);

    if(box_label.box_[2] < 0.05 || box_label.box_[3] < 0.05)
      continue;
