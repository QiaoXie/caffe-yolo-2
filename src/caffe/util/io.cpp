...
bool ReadDecDataToDatum(const string& filename, const string& annoname, 
	const int height, const int width, 
  ...
  if (cv_img.data) {
    if (encoding.size()) {
      ...
      datum->set_encoded(true);
      // read xml anno data
      LabelBoxToDatum(annoname, datum);
      return true;
    }
    CVMatToDatum(cv_img, datum);
    // read xml anno data
...
void LabelBoxToDatum(const string& annoname, Datum* datum)
{

	int label_ind = 0;
  vector<float> box(4, 0);
	string line;
	ifstream ann_file(annoname.c_str());
  datum->clear_float_data();
	while(getline(ann_file, line))
	{
		sscanf(line.c_str(), "%d %f %f %f %f", &label_ind, &box[0], &box[1], &box[2], &box[3]);
		datum->add_float_data(float(label_ind));
		for (int i = 0; i < 4; ++i) {
...
bool ReadFileToDatum(const string& filename, const string& annoname, Datum* datum) 
{
  std::streampos size;
